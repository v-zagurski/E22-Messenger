...
class Config:
    ...
    def set_all(self, data: bytearray):
        assert len(data) == 9
    ...
    def set_transmitting_power(self, power=3):
        ...
        self.reg1[0] = set_bits(self.reg1[0],
                                [1, 0],
                                [True if power & (1 << (1 - n)) else False for n in range(2)])
...
class E22:
    ...
    def get_rssi_env_noise(self, delay=0.2):
        ...
        return f'RSSI : Ambient={-data[3]/2}'
    ...
    def software_mode_switch(self, mode, delay=0.1):
        ...
        if not len(data) == 5:
            raise Exception('software_mode_switch: software mode switch not enabled', data)
